...
int main(void)//双机通信之主机，完成整体计算，插在右边卡槽
{
  ...
  //    Test_GPIO_EXIT_Init(); //PASS,测试外部中断、PB10和PA3/PC4闪灯
  //    Test_GPIO_LED();       //PASS,测试GPIO，PB10和PA3/PC4闪灯
  //    Test_GPIO_KEY();       //PASS,测试外部按键输入，PC123   按下按键 LED亮
  //    Test_ComKEY_Tft();     //PASS,测试外部组合按键输入并TFT1.8显示，PC123
  //    TestEncoderOLED();     //PASS,测试编码器正交解码,OLED和UART输出
  //    TestEncoderTFT();      //PASS,测试编码器正交解码,TFT1.8和UART输出
                               /*编码器测试说明：测试编码器首先通过红定义打开LQ_Config.h文件，选择宏定义defined  DOUBLE_CORE_SLAVE，烧录到左侧核心板
                                                                                                                                      再换回宏定义#define DOUBLE_CORE_HOST，烧录到右侧核心板，再TFT1.8屏幕上显示4路编码器读取值。*/
  //    Test_OLED();           //PASS,测试OLED0.96屏使用P20.14--10，显示字符串及动态数据
  //    Test_TFT18();          //PASS,测试TFT1.8彩屏使用P20.14--10，显示字符串及动态数据
  //    Test_Bluetooth();      //PASS,测试UART1(PA10RX/PA9TX,直接用串口助手和LQLINKRV)
  //    Test_EEPROM();         //PASS,测试内部EEPROM擦写功能  OLED提示是否写入成功
  //    Test_CAMERA();         //PASS，测试摄像头函数，核心板插在左侧位置，拔掉右侧核心板，通过TFT1.8屏幕显示摄像头采集画面
  //    ElectroMagneticMWCar();//PASS,麦轮电磁主程序待完善，请关注更新，需要另外一个核心板提供编码器数据
