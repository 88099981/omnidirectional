...
float piderror, pidlast_error, pidlast_derivative;
float pidp = 40, pidi = 0, pidd = 0;
float pidout_p = 0, pidout_i = 0, pidout_d = 0, pidout = 0;
...
void SpeedCtrl(signed short x, signed short y, signed short z)
{
    ...
    // 获取编码器值
    realSpeed1 = - Read_Encoder(2); //左电机 母板上编码器1，小车前进为负值
    realSpeed2 = - Read_Encoder(4); //右电机 母板上编码器2，小车前进为正值
    realSpeed3 = ECPULSE3/10; //另外一个核心板发数据过来
    realSpeed4 = ECPULSE4/10; //另外一个核心板发数据过来
   ...
   /* piderror = targetSpeed1 - realSpeed1;
    ...
    MotorCtrl4w(pidout + targetSpeed1, targetSpeed2, targetSpeed3, targetSpeed4);
