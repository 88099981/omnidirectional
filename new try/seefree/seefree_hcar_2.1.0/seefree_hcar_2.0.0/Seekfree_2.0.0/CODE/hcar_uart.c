...
void data_analysis(uint8 *line)
{
    ...
    if(line[10]== 0xB3)    fuhe_speed          = ((int16)line[11] << 8) | line[12];
    if(line[13]== 0xB4)    fuhe_sancha         = (int8)line[14];
    if(line[15]== 0xB5)    fuhe_ad_all         = ((int16)line[16] << 8) | line[17];
}
...
void process_data(void)
{
    ...
    temp_buff[11] = ad_speed>>8;            //数据高8位
    temp_buff[12] = ad_speed&0xFF;          //数据低8位

    temp_buff[13] = 0xB4;                         //功能字
    temp_buff[14] = three_fork_flag;            //数据高8位

    temp_buff[15] = 0xB5;                         //功能字
    temp_buff[16] = ad_all>>8;            //数据高8位
    temp_buff[17] = ad_all&0xFF;          //数据低8位

    temp_buff[18] = 0xEE;                        //帧尾
}
