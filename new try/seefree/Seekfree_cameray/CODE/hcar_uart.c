...
{
    if(line[1] == 0xB0)    sum_x1  = ((int16)line[2] << 8) | line[3];
    if(line[4] == 0xB1)
    {
        l_turn_flag = line[5];
        r_turn_flag = line[6];
    ...
    if(line[7] == 0xB2)    sum_x2  = ((int16)line[8] << 8) | line[9];
    if(line[10] == 0xB3)
    {
        cross_flag=line[11];
    }
...
//    slave_encoder_right--;
    //slave_position++;
    slave_encoder_left=sum_x1;
    slave_position=sum_x2;
...
void process_data(void)
{
    ...
    temp_buff[5] = l_turn_flag;       //数据高8位
    temp_buff[6] = r_turn_flag;     //数据低8位

    temp_buff[7] = 0xB2;                         //功能字
    temp_buff[8] = slave_position>>8;            //数据高8位
    temp_buff[9] = slave_position&0xFF;          //数据低8位

    temp_buff[10] = 0xB3;            //数据高8位
    temp_buff[11] = cross_flag;          //数据低8位
