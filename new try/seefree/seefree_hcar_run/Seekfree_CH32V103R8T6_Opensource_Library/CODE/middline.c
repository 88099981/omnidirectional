    ...
    for(edge_i= EDGE_START;edge_i>15;edge_i--)
    {
        ...
        for(edge_j=1;edge_j<col_end;edge_j++)//找左边沿
        {
            ...
            if(img[edge_i][edge_j]==Black && img[edge_i][edge_j+1]==White && img[edge_i][edge_j+2]==White  && img[edge_i][edge_j+4]==White   && img[edge_i][edge_j+5]==White)//找到黑白跳变点
            {
                ...
                else if (new_l_edge[edge_i + 2] <= 5 && new_r_edge[edge_i + 2] >= IMG_W - 5 )//对于十字特殊情况
                {
                    l_edge_limt_flag = 1;//开启屏蔽等待下次校正
                    if((new_l_edge[EDGE_START-2 ] !=0 && new_r_edge[EDGE_START-2] >= IMG_W - 5)||(new_l_edge[EDGE_START-2 ] <=col_start && new_r_edge[EDGE_START-2] !=IMG_W))
                    {}
                    else
                    {
                        l_edge_limt_flag = 1;//开启屏蔽等待下次校正
...
//        ok_sign=3;
//    }
    if(l_cross_flag==1 && r_cross_flag==1)
    {
       cross_flag=1;
    }
...
//        lasttime_x=sum_x1;
        ...
        aim_z = kp_z*z_error + kd_z*(z_error-lasttime_z);
        lasttime_z=z_error;
